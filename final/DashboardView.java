...
/**
 * DashboardView.java — Main Application Shell
 * ============================================
 * Pure-Java JavaFX layout implementing a three-zone shell:
 *
 *   ┌──────────┬────────────────────────────────┐
 ...
 *   │  NAV     │                                │
 *   │          │    CENTER WORKSPACE            │
 *   │          │    (swapped by nav clicks)     │
 *   └──────────┴────────────────────────────────┘
 *
 * Each sidebar item maps to a workspace builder function stored in a
 * LinkedHashMap. Clicking a nav item replaces only the center region —
 * sidebar and topbar remain static (no scene rebuild cost).
 *
 * To add a new workspace panel (e.g. Course, Club):
 *   1. Add a new NavItem constant in the inner enum.
 *   2. Register its builder lambda in buildWorkspaceRegistry().
 *   3. Done — the sidebar auto-populates.
 */
public class DashboardView {
    ...
    /** Ordered nav entries — icon (emoji), label, identifier key. */
    ...
    /** Maps nav key → workspace node builder. */
    private final Map<String, java.util.function.Supplier<Node>> workspaceRegistry;

    /** The currently active nav button, used to toggle the highlight. */
    private Button activeNavButton;

    /** Mutable breadcrumb label in the top bar. */
    private Label breadcrumbLabel;

    /** The center region — replaced on each nav click. */
    ...
    public DashboardView(String loggedInUser, Runnable onLogout) {
        ...
        // Build workspace registry before constructing layout
    ...
    // ------------------------------------------------------------------
    // Workspace registry — maps nav keys to builder functions
    // ------------------------------------------------------------------

    private Map<String, java.util.function.Supplier<Node>> buildWorkspaceRegistry() {
        // LinkedHashMap preserves insertion order (matches NavItem enum order)
        Map<String, java.util.function.Supplier<Node>> reg = new LinkedHashMap<>();

        // Each builder is called lazily only when user navigates to that panel
    ...
    private BorderPane buildLayout() {
        ...
        // Build sub-regions (order matters for cross-references)
    ...
    /**
     * Replaces the center workspace by calling the registered builder.
     * Only the center region is touched — sidebar and topbar stay intact.
     */
    ...
    // ------------------------------------------------------------------
    // Generic placeholder (for panels not yet implemented by teammates)
    // ------------------------------------------------------------------
